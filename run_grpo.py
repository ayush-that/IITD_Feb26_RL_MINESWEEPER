...
print(f"GRPO dataset: {len(grpo_dataset)} examples (filtered {skipped_long} long prompts)")
...
for rows, cols, mines, n_games in board_configs:
    ...
    print(f"  {rows}x{cols}: JSON={json_rate:.0f}% ValidMove={move_rate:.0f}% Wins={wins}/{n_games} ({total_moves} moves)")
