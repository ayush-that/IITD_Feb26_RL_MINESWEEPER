...
print(f"Model loaded. Trainable: {sum(p.numel() for p in model.parameters() if p.requires_grad)/1e6:.1f}M")
...
for ex in raw_data:
    ...
    board = json.loads(ex['board_state']) if isinstance(ex['board_state'], str) else ex['board_state']
    flagged_pos = json.loads(ex['flagged_positions']) if isinstance(ex['flagged_positions'], str) else ex['flagged_positions']
...
print(f"GRPO dataset: {len(grpo_dataset)} examples ({rebuilt_count} rebuilt to frontier, {skipped_long} filtered long)")
    ...
    for idx, completion in enumerate(completions):
        ...
        try:
            ...
            if not (0 <= row < rows and 0 <= col < cols):
                scores.append(-15.0 / 25.0); continue
            if (row, col) in revealed_set:
                scores.append(-12.0 / 25.0); continue
            if (row, col) in flagged_set:
                scores.append(-8.0 / 25.0); continue
            if action_type == "flag":
                if len(flagged_set) >= num_mines:
                    scores.append(-10.0 / 25.0); continue
...
def strategic_reward(completions, **kwargs):
    ...
    for idx, completion in enumerate(completions):
        ...
        try:
            ...
            action_is_deducible = any(m[0] == action_type and m[1] == row and m[2] == col for m in deducible_raw)
            if has_deducible and not action_is_deducible:
                score -= 0.3
            adjacent_to_number = any(
                0 <= row+dr < rows and 0 <= col+dc < cols and board[row+dr][col+dc] in '012345678'
...
for rows, cols, mines, n_games in board_configs:
    ...
    for seed in range(n_games):
        ...
        for move_i in range(min(30, rows * cols)):
            ...
            messages = [{"role": "system", "content": sys_prompt_eval}, {"role": "user", "content": prompt}]
    ...
    print(f"  {rows}x{cols}: JSON={json_rate:.0f}% Valid={move_rate:.0f}% Wins={wins}/{n_games} Score={avg_score:.1f} (safe={reveal_safe} mine={reveal_mine} flag+={flag_correct} flag-={flag_wrong})")
...
base_model = AutoModelForCausalLM.from_pretrained(base_path, torch_dtype=torch.bfloat16, device_map="cpu")
...
total_size = sum(os.path.getsize(os.path.join(output_path, f)) for f in os.listdir(output_path) if os.path.isfile(os.path.join(output_path, f)))
